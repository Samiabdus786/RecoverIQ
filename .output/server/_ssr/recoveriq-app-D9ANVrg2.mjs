...
//#region node_modules/.nitro/vite/services/ssr/assets/recoveriq-app-D9ANVrg2.js
	...
	async function action(type, id) {
		...
		try {
			if (type === "execute" && id) {
				...
				if (!casesResponse.ok) throw new Error("Could not load real recovery cases");
				const casesJson = await casesResponse.json();
				const realCase = (Array.isArray(casesJson) ? casesJson : casesJson.cases || []).find((c) => c.external_id === id);
				if (!realCase) throw new Error(`Backend recovery case not found for ${id}`);
				const executeResponse = await fetch(`${API_BASE}/recovery/${realCase.id}/execute`, {
					method: "POST",
					headers: { "content-type": "application/json" }
				});
				...
				const latestResponse = await fetch(`${API_BASE}/recovery/${realCase.id}`);
				let latestCase = {};
				if (latestResponse.ok) latestCase = await latestResponse.json();
				console.log("Latest backend case:", latestCase);
				const paymentUrl = executeResult.payment_link_url || executeResult.paymentLinkUrl || executeResult.short_url || executeResult.payment_url || executeResult.provider_url || executeResult.url || latestCase.payment_link_url || latestCase.paymentLinkUrl || latestCase.short_url || latestCase.payment_url || latestCase.provider_url || latestCase.url;
				if (!paymentUrl) throw new Error("Razorpay payment link was created but no payment URL was returned.");
				setData((current) => {
					if (!current) return current;
					return {
						...current,
						provider: "RAZORPAY TEST MODE",
						cases: current.cases.map((c) => c.id === id ? {
							...c,
							state: "MONITORING",
							paymentLinkUrl: paymentUrl,
							providerRef: latestCase.provider_ref || latestCase.providerRef || executeResult.provider_ref || executeResult.providerRef || c.providerRef,
							operationStatus: "Razorpay Test payment link created successfully."
						} : c)
					};
				});
			...
			setError(err instanceof Error ? err.message : "The recovery action could not be completed.");
		} finally {
			setWorking("");
		}
	}
