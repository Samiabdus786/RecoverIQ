...
var __vite_rsc_assets_manifest_default = {
	"bootstrapScriptContent": "import(\"/assets/index-C8G52JQk.js\")",
	"clientReferenceDeps": {
		"195fc1a01ae3": {
			"js": [
				"/assets/recoveriq-app-BndAlNNp.js",
				"/assets/rolldown-runtime-S-ySWqyJ.js",
				"/assets/framework-CXnKph_e.js",
				"/assets/index-C8G52JQk.js"
			],
			"css": []
		},
		"593f344dc510": {
		...
		"15c18cfaeeff": {
			"js": [
				"/assets/layout-segment-context-PQ5gcrd_.js",
				"/assets/rolldown-runtime-S-ySWqyJ.js",
				"/assets/index-C8G52JQk.js",
				"/assets/framework-CXnKph_e.js"
			],
...
function getDraftSecret() {
	return "2ec9b268-02c3-486c-aa50-5d327cecb14e";
}
/**
...
var NextURL = class NextURL {
	...
	*/
	get buildId() {
		return "d23abec9-b657-48b3-a211-190d92818509";
	}
};
...
*/
function appIsrCacheKey(pathname, suffix, buildId = "d23abec9-b657-48b3-a211-190d92818509") {
...
function createAppPageArtifactCompatibility(element, routePattern) {
	...
	return createArtifactCompatibilityEnvelope({
		...
		deploymentVersion: "d23abec9-b657-48b3-a211-190d92818509",
		rootBoundaryId
	});
}
/**
...
var recoveriq_app_default = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'default' is called on server");
}, "195fc1a01ae3", "default");
var page_exports$7 = /* @__PURE__ */ __exportAll({ default: () => Home });
function Home() {
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(recoveriq_app_default, {});
}
...
var page_exports$6 = /* @__PURE__ */ __exportAll({ default: () => AnalyticsPage });
function AnalyticsPage() {
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(recoveriq_app_default, { initialView: "analytics" });
}
var route_exports$4 = /* @__PURE__ */ __exportAll({ POST: () => POST$3 });
async function POST$3(request) {
	const body = await request.json().catch(() => null);
	if (!body?.accessToken) return NextResponse.json({ error: "Google access token is required" }, { status: 400 });
	const response = await fetch("https://www.googleapis.com/oauth2/v3/userinfo", {
		headers: { authorization: `Bearer ${body.accessToken}` },
		cache: "no-store"
	});
	if (!response.ok) return NextResponse.json({ error: "Google sign-in could not be verified" }, { status: 401 });
	const profile = await response.json();
	if (!profile.email || profile.email_verified !== true) return NextResponse.json({ error: "Google account email is not verified" }, { status: 403 });
...
function seededAudit() {
	return original.slice(0, 7).map((item, i) => ({
		...
		detail: item.method === "Checkout" ? `${item.failure} detected before payment completion.` : `${item.failure} detected on ${item.method} payment.`,
		result: "DETECTED"
	}));
}
function store() {
...
function resetDemo() {
	const settings = globalStore.recoveriqDemo?.settings ?? { ...DEFAULT_DEMO_SETTINGS };
	globalStore.recoveriqDemo = {
		cases: structuredClone(original),
		audit: seededAudit(),
		batchRuns: 0,
...
function updateSettings(input) {
	...
	store().settings = {
		autoRecovery: typeof input.autoRecovery === "boolean" ? input.autoRecovery : current.autoRecovery,
		approvalThreshold: Number.isFinite(threshold) ? Math.min(1e5, Math.max(5e3, Math.round(threshold / 5e3) * 5e3)) : current.approvalThreshold,
		maxAttempts: Number.isFinite(attempts) ? Math.min(4, Math.max(1, Math.round(attempts))) : current.maxAttempts,
		maxContacts: Number.isFinite(contacts) ? Math.min(4, Math.max(0, Math.round(contacts))) : current.maxContacts,
		recoveryWindowHours: Number.isFinite(windowHours) ? Math.min(336, Math.max(24, Math.round(windowHours / 24) * 24)) : current.recoveryWindowHours
	};
	store().audit.unshift({
		time: now(),
		caseId: "POLICY",
...
function log(item, component, event, detail, result) {
	store().audit.unshift({
		time: now(),
		caseId: item.id,
		component,
...
function configuredProviderName() {
	return process.env.PAYMENT_PROVIDER === "razorpay" && process.env.RAZORPAY_KEY_ID && process.env.RAZORPAY_KEY_SECRET ? "RAZORPAY TEST MODE" : "MOCK PROVIDER";
}
function configuredAiMode() {
	return process.env.GEMINI_API_KEY ? "GEMINI LIVE DIAGNOSIS / DETERMINISTIC GUARDRAILS" : "DEMO / DETERMINISTIC AI MODE";
}
function fallbackDiagnosis(item) {
	return {
		cause: item.failure,
		summary: `${item.action.replaceAll("_", " ")} selected from recovery score and failure reason.`,
		recommended_action: item.action,
		confidence: Math.min(.94, Math.max(.55, item.probability + .08)),
...
async function diagnoseWithGemini(item) {
	...
	try {
		...
		if (!diagnosis.summary || !diagnosis.recommended_action || !SAFE_ACTIONS.has(diagnosis.recommended_action)) throw new Error("Gemini returned an unsafe diagnosis shape");
		return {
			diagnosis,
			provider: "GEMINI",
			fallback: false
...
async function createProviderAction(item) {
	if (item.action !== "CREATE_PAYMENT_LINK" || configuredProviderName() !== "RAZORPAY TEST MODE") {
		item.providerRef = `plink_mock_${item.id.slice(-3)}`;
		return {
			ok: true,
			detail: item.action === "CREATE_PAYMENT_LINK" ? "Mock provider created one idempotent recovery action." : "Demo action recorded without a payment link.",
			result: "MONITORING"
		};
	}
	const auth = btoa(`${process.env.RAZORPAY_KEY_ID}:${process.env.RAZORPAY_KEY_SECRET}`);
	const providerReferenceId = `${item.id}-${Date.now().toString(36)}`.slice(0, 40);
	const response = await fetch("https://api.razorpay.com/v1/payment_links", {
		...
		body: JSON.stringify({
			...
			notes: { recoveriq_reference: item.id.slice(0, 40) }
		})
	});
	if (!response.ok) {
		let reason = `HTTP ${response.status}`;
		try {
			reason = (await response.json()).error?.description || reason;
		} catch {}
		return {
			ok: false,
...
async function executeAuthorizedAction(item, detail) {
	if (item.paymentLinkUrl) {
		item.operationStatus = "Payment link already exists. Duplicate creation blocked.";
		log(item, "GUARDRAIL", "DUPLICATE_LINK_BLOCKED", "Existing payment link retained; no duplicate Razorpay action was created.", "BLOCKED");
		return;
	}
	if ((item.ageHours ?? 24) > store().settings.recoveryWindowHours) {
		item.state = "STOPPED";
		item.operationStatus = "Recovery window expired. Workflow stopped.";
		log(item, "GUARDRAIL", "OUTSIDE_RECOVERY_WINDOW", "Configured recovery window expired before execution.", "BLOCKED");
		return;
	}
	if ((item.attempts ?? 0) >= store().settings.maxAttempts) {
		item.state = "STOPPED";
		item.operationStatus = "Maximum recovery attempts reached. Workflow stopped.";
		log(item, "GUARDRAIL", "MAX_ATTEMPTS_REACHED", "Configured attempt ceiling blocked another recovery action.", "BLOCKED");
		return;
	}
	if ([
		"CREATE_PAYMENT_LINK",
		"SEND_REMINDER",
		"SUGGEST_ALTERNATE_METHOD"
	].includes(item.action) && (item.attempts ?? 0) >= store().settings.maxContacts) {
		item.state = "STOPPED";
		item.operationStatus = "Maximum customer contacts reached. Workflow stopped.";
		log(item, "GUARDRAIL", "MAX_CONTACTS_REACHED", "Configured contact ceiling blocked another customer-facing action.", "BLOCKED");
		return;
	}
	...
	} else item.operationStatus = provider.detail;
	log(item, "PROVIDER", provider.ok ? "PAYMENT_LINK_CREATED" : "PAYMENT_LINK_CREATION_ATTEMPT", provider.detail, provider.result);
}
async function runBatch() {
	...
	for (const item of s.cases) {
		...
		log(item, "ML", "RECOVERY_SCORED", `XGBoost probability ${Math.round(item.probability * 100)}%.`, "SCORED");
		const ai = await diagnoseWithGemini(item);
		const recommendation = ai.diagnosis.recommended_action || item.action;
		if (SAFE_ACTIONS.has(recommendation) && item.priority !== "SUPPRESSED" && recommendation !== "HUMAN_APPROVAL") item.action = recommendation;
		log(item, "AI", ai.fallback ? "ACTION_SELECTED_FALLBACK" : "GEMINI_DIAGNOSIS", `${ai.provider}: ${ai.diagnosis.summary || item.action}; action ${item.action}; expected value ₹${item.expectedValue.toLocaleString("en-IN")}.`, ai.fallback ? "FALLBACK" : "RECOMMENDED");
		if (item.priority === "SUPPRESSED") {
			item.state = "STOPPED";
			log(item, "GUARDRAIL", "SAFE_SUPPRESSION", "Intervention suppressed; no customer contact.", "APPROVED");
		} else if (!s.settings.autoRecovery || item.amount >= s.settings.approvalThreshold) {
			item.state = "AWAITING_APPROVAL";
			item.guardrail = "HUMAN_REVIEW";
			log(item, "GUARDRAIL", "HUMAN_APPROVAL_REQUIRED", !s.settings.autoRecovery ? "Automatic execution disabled by merchant policy." : `High-value threshold ₹${s.settings.approvalThreshold.toLocaleString("en-IN")} crossed.`, "PAUSED");
		} else await executeAuthorizedAction(item, "Deterministic rules approved bounded execution.");
	}
	return snapshot();
}
async function act(type, id) {
	...
	if (type === "execute" && item.paymentLinkUrl) {
		item.operationStatus = "Payment link already exists. Duplicate creation blocked.";
		log(item, "GUARDRAIL", "DUPLICATE_LINK_BLOCKED", "Existing payment link retained; no duplicate Razorpay action was created.", "BLOCKED");
	} else if (type === "execute" && ["DETECTED", "FAILED"].includes(item.state)) if (item.priority === "SUPPRESSED" || item.action === "DO_NOTHING") {
		item.state = "STOPPED";
		log(item, "GUARDRAIL", "SAFE_SUPPRESSION", "Intervention suppressed; no customer contact.", "APPROVED");
	} else if (!store().settings.autoRecovery || item.amount >= store().settings.approvalThreshold) {
		item.state = "AWAITING_APPROVAL";
		item.guardrail = "HUMAN_REVIEW";
		log(item, "GUARDRAIL", "HUMAN_APPROVAL_REQUIRED", !store().settings.autoRecovery ? "Automatic execution disabled by merchant policy." : `High-value threshold ₹${store().settings.approvalThreshold.toLocaleString("en-IN")} crossed.`, "PAUSED");
	} else await executeAuthorizedAction(item, "Merchant manually triggered an eligible recovery action.");
	else if (type === "approve" && item.state === "AWAITING_APPROVAL") {
		if ((item.attempts ?? 0) >= store().settings.maxAttempts) throw new Error("Maximum recovery attempts reached");
		log(item, "MERCHANT", "HUMAN_APPROVAL", "Merchant approved the high-value recovery action.", "APPROVED");
		await executeAuthorizedAction(item, "Action executed only after approval.");
	} else if (type === "reject" && item.state === "AWAITING_APPROVAL") {
		item.state = "STOPPED";
		item.operationStatus = "Merchant rejected the proposed recovery action.";
		log(item, "MERCHANT", "HUMAN_APPROVAL_REJECTED", "Merchant rejected the high-value recovery action; workflow stopped.", "REJECTED");
		log(item, "GUARDRAIL", "STOPPING_RULE_APPLIED", "No customer contact or provider action is allowed after rejection.", "STOPPED");
		...
		log(item, "WEBHOOK", "PAYMENT_CAPTURED", "Payment recovered; workflow stopped immediately.", "RECOVERED");
		log(item, "GUARDRAIL", "STOPPING_RULE_APPLIED", "No further contacts or actions are allowed.", "STOPPED");
	} else if (type === "failure") {
		if (item.paymentLinkUrl || item.state === "RECOVERED") {
			item.operationStatus = "Provider failure test ignored because a payment link already exists or payment is complete.";
			log(item, "PROVIDER", "PROVIDER_FAILURE_IGNORED", "Existing successful or monitoring payment flow retained; case state was not changed.", "IGNORED");
			return snapshot();
		}
		item.state = "FAILED";
		item.operationStatus = "Provider failure simulated safely before any successful payment link was created.";
		log(item, "PROVIDER", "PROVIDER_FAILURE", "Simulated timeout; action key retained and no duplicate created.", "SAFE_FAILURE");
		...
		log(item, "WEBHOOK", "DUPLICATE_IGNORED", "Repeated event ID ignored; 0 duplicate actions.", "IGNORED");
	}
	return snapshot();
}
function getRazorpayReference(payload) {
	const body = payload;
	const paymentNotes = body.payload?.payment?.entity?.notes;
	const linkNotes = body.payload?.payment_link?.entity?.notes;
	const reference = paymentNotes?.recoveriq_reference || linkNotes?.recoveriq_reference;
	return typeof reference === "string" ? reference : "";
}
function processRazorpayWebhook(payload, eventId) {
	const s = store();
	...
	if (item && ["payment.captured", "payment_link.paid"].includes(event) && item.state !== "RECOVERED") {
		item.state = "RECOVERED";
		...
		log(item, "WEBHOOK", "RAZORPAY_PAYMENT_VERIFIED", "Signed Razorpay webhook matched this recovery case.", "RECOVERED");
		log(item, "GUARDRAIL", "STOPPING_RULE_APPLIED", "No further customer contact or provider action is allowed.", "STOPPED");
	} else if (item) {
		item.webhookStatus = `Razorpay webhook ${event} recorded without recovery state change.`;
		log(item, "WEBHOOK", "RAZORPAY_EVENT_RECORDED", item.webhookStatus, "RECORDED");
	}
	return {
		status: "processed",
		eventId,
...
function snapshot() {
	...
	const atRisk = s.cases.filter((c) => c.state !== "RECOVERED").reduce((a, c) => a + c.amount, 0);
	const revenue = recovered.reduce((a, c) => a + c.amount, 0);
	const cost = s.cases.filter((c) => c.state === "MONITORING" || c.state === "RECOVERED" || c.state === "FAILED").length * 3.5;
	return {
		...
		settings: s.settings,
		policyImpact: {
			automaticCases: s.settings.autoRecovery ? s.cases.filter((c) => c.priority !== "SUPPRESSED" && c.amount < s.settings.approvalThreshold).length : 0,
			reviewCases: s.cases.filter((c) => c.priority !== "SUPPRESSED" && (!s.settings.autoRecovery || c.amount >= s.settings.approvalThreshold)).length,
			suppressedCases: s.cases.filter((c) => c.priority === "SUPPRESSED").length,
			expectedValue: s.cases.filter((c) => c.priority !== "SUPPRESSED").reduce((sum, c) => sum + c.expectedValue, 0)
		},
		metrics: {
			atRisk,
			recovered: revenue,
		...
		timeline: [
			...
			},
			{
				day: "Aug 26",
				risk: 110300,
				recovered: revenue * .48
			},
			{
				day: "Aug 27",
				risk: 136200,
				recovered: revenue * .72
			},
			{
				day: "Aug 28",
				risk: atRisk,
...
async function POST$2(request) {
	try {
		...
		return NextResponse.json(await act(body.type, body.id));
	} catch (error) {
...
async function POST(request) {
	...
	if (!safeEqual(request.headers.get("x-razorpay-signature") || "", await hmacSha256Hex(body, secret))) return NextResponse.json({ error: "Invalid Razorpay webhook signature" }, { status: 401 });
	const eventId = request.headers.get("x-razorpay-event-id") || `sha256:${await sha256Hex(body)}`;
	let payload;
	try {
...
var page_exports$5 = /* @__PURE__ */ __exportAll({ default: () => AuditPage });
function AuditPage() {
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(recoveriq_app_default, { initialView: "audit" });
}
var page_exports$4 = /* @__PURE__ */ __exportAll({ default: () => DashboardPage });
function DashboardPage() {
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(recoveriq_app_default, { initialView: "overview" });
}
var page_exports$3 = /* @__PURE__ */ __exportAll({ default: () => RecoveryPage });
function RecoveryPage() {
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(recoveriq_app_default, { initialView: "queue" });
}
var page_exports$2 = /* @__PURE__ */ __exportAll({ default: () => SettingsPage });
function SettingsPage() {
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(recoveriq_app_default, { initialView: "settings" });
}
var page_exports$1 = /* @__PURE__ */ __exportAll({ default: () => TransactionsPage });
function TransactionsPage() {
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(recoveriq_app_default, { initialView: "queue" });
}
var page_exports = /* @__PURE__ */ __exportAll({ default: () => TransactionPage });
...
var routes = [
	...
	},
	{
		...
		ids: {
			"route": "route:/api/auth/google",
			"page": null,
			"routeHandler": "route-handler:/api/auth/google",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			"route": "route:/api/demo/action",
			"page": null,
			"routeHandler": "route-handler:/api/demo/action",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/demo/action",
		patternParts: [
			"api",
			"demo",
			"action"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/demo/settings",
			"page": null,
			"routeHandler": "route-handler:/api/demo/settings",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/demo/settings",
		patternParts: [
			"api",
			"demo",
			"settings"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/demo/state",
			"page": null,
			"routeHandler": "route-handler:/api/demo/state",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/demo/state",
		patternParts: [
			"api",
			"demo",
			"state"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/webhooks/razorpay",
			"page": null,
			"routeHandler": "route-handler:/api/webhooks/razorpay",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
...
var generateStaticParamsMap = { "/transactions/:id": null };
